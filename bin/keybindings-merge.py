...
python3 keybindings-merge.py [--prefer {left,right}] [--base {left,right}] left.json right.json [--out merged.json]

Examples:

python3 keybindings-merge.py fileA.json fileB.json
python3 keybindings-merge.py --prefer left --base right a.json b.json --out merged.json

Description:

Merge two VS Code `keybindings.json` (JSONC) files while preserving ALL comments
and the original raw text of each top-level array item. Items are deduplicated by
the pair "<key>|<when>". Use `--prefer` to choose which file wins on duplicates
(default: `right`) and `--base` to choose which file provides the surrounding
wrapper/prefix/suffix (default: `left`). Unparsable items are preserved verbatim
and are not deduplicated.

Notes:
- Requires Python 3.7 or newer.
- Does not remove or alter comments; the merged output keeps the original item
    text and spacing.
- Writes output to the `--out` path (default: `merged-keybindings.json`).
- Warnings are printed for items that could not be parsed.
"""
