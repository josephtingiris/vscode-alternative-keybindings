...
-- Layout orientation is a visual/layout preference that references the position of the Primary Side Bar. By default the Primary Side Bar is on the left side of the Editor Group (left orientation). If the Primary Side Bar is moved to the right side of the Editor Group, that is a right-side orientation. This preference should be considered when designing and implementing directional navigation keybindings to ensure they feel natural and intuitive for either left- or right-side sidebar orientations. Essentially, the 'next' and 'previous' navigation keybindings should adapt based on the sidebar orientation of the layout to maintain a consistent user experience.
